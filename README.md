...
- 多适配器支持：内置 Gemini、OpenAI Chat、OpenAI Images、火山方舟、Gitee AI、SiliconFlow、Agnes AI、Jimeng2API、Grok 和自定义 HTTP 接口。
...
- 提示词分块：同时使用人设、预设和附加提示词时，会按来源整理为轻量分块，提升模型理解稳定性。
...
| 最大并发完整生图任务数 | 1      | 同时运行的完整生图任务数量上限；每个运行中任务内部仍受请求级并发限制控制。                     |
...
- 运行时重载“最大并发生图请求数”会逐步对新请求生效，旧请求可能仍持有旧的请求级限制器。
