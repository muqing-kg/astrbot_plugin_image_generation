...
开始生图任务提示模板支持这些占位符：`{reference_image_count}`、`{prompt}`、`{preset}`、`{presets}`、`{persona}`、`{personas}`、`{aspect_ratio}`、`{resolution}`、`{task_id}`、`{model}`、`{mode}`、`{reference_images_block}`、`{preset_block}`、`{persona_block}`。其中 `{preset}`/`{presets}` 仅表示命中的预设，`{persona}`/`{personas}` 仅表示命中的人设。

### 任务体系

`/生图` 指令和 LLM 生图工具共用统一任务体系：任务提交后会生成 `task_id`，进入排队/运行/完成/失败/取消状态，并受最大并发生图任务数限制。`/生图任务` 只展示当前会话正在进行的任务，并为列表项添加编号；可通过 `/生图任务 <编号或任务ID>` 查看详情，通过 `/生图取消 <编号或任务ID>` 取消仍在排队或运行中的任务。任务记录保存在内存中，插件重载后会清空；已结束任务不会在列表中展示。
