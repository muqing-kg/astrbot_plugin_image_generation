...
- 提示词审核和图片审核仍按插件配置执行
- `task_id` 由 API 自动生成
- 提交任务会在收集参考图和预约额度前预检查队列容量；队列满时直接返回 `queue_full`
- 任务历史元数据默认会持久化，插件重载后最近的已结束任务仍可查询；重启前未完成的任务会恢复为 `cancelled`，不会继续执行
- 历史持久化内容包括会话标识、提示词摘要、结果路径和错误摘要，可通过“任务持久化”配置关闭或调整保留条数/天数
